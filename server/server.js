...
app.post('/', async (req, res) => {
  try {
    ...
    const sectionCheck = await openai.createCompletion({
      ...
      prompt: "Sort this request " + prompt + " into one of these categories: advantages, different models, Contact, Discounts, Shipment. Respond with only the section name",
      temperature: 0.1,
    })
